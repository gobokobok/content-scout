...
async def test_shortlist_history_empty(session: AsyncSession) -> None:
    owner, project, *_ = await _setup(session)

    async with await client(session) as c:
        resp = await c.get(
            f"/projects/{project.id}/history/shortlist",
...
async def test_shortlist_history_shows_added_items(session: AsyncSession) -> None:
    ...
    async with await client(session) as c:
        ...
        resp = await c.get(f"/projects/{project.id}/history/shortlist", headers=headers)
        assert resp.status_code == 200
        body = resp.json()
        assert len(body) == 2
        # All active: removed_at is None
        assert all(entry["removed_at"] is None for entry in body)
        ids = {entry["content_item_id"] for entry in body}
        assert str(item1.id) in ids and str(item2.id) in ids


async def test_shortlist_history_shows_removed_items(session: AsyncSession) -> None:
    owner, project, run, item1, item2 = await _setup(session)

    async with await client(session) as c:
        headers = auth_headers(owner.id)

        # Add both, remove item1
        await c.post(
            f"/projects/{project.id}/shortlist/items",
            headers=headers,
            json={"item_ids": [str(item1.id), str(item2.id)]},
        ...
        by_id = {entry["content_item_id"]: entry for entry in body}
        # item1 was removed — removed_at is set
        assert by_id[str(item1.id)]["removed_at"] is not None
        # item2 is still active
        assert by_id[str(item2.id)]["removed_at"] is None


async def test_shortlist_history_shape(session: AsyncSession) -> None:
    ...
    async with await client(session) as c:
        ...
        assert entry["account_handle"] == "natgeo"
        assert entry["type"] == "reel"
        assert entry["title"] == "Reel A"


async def test_shortlist_history_404_for_wrong_user(session: AsyncSession) -> None:
    owner, project, *_ = await _setup(session)
