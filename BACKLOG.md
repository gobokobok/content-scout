...
- **CI caught a real issue after the first push:** the new test's extra `/auth/register` call tipped `test_auth.py` over `middleware/rate_limit.py`'s shared 10-per-minute-per-path counter (CI's Redis isn't reset between tests in a file) — `test_update_display_name_rejects_too_long` failed clean in CI with a silent 429 (`KeyError: 'access_token'`). Fixed with an autouse `_bypass_rate_limit` fixture in `test_auth.py` (same `_noop_rate_limit` pattern `test_guardrails.py` already uses), since this file was never meant to exercise rate-limiting itself. Confirmed stable across 3 repeat runs and the full 320-test suite.
...
### Acceptance Criteria
- [x] Deleting a competitor that has ever appeared in a completed run succeeds instead of hitting an unhandled `IntegrityError`
- [x] Removing a competitor preserves its scrape history (`content_items`, `shortlist_items`, `deep_analysis_items`) rather than deleting it
- [x] Re-adding the same competitor URL after removal reactivates the same account (same id, history intact) instead of erroring on the unique constraint
- [x] The run-creation competitor picker always reflects the current competitor list, not a stale page-load-time snapshot
- [x] The competitor picker's list scrolls correctly when it exceeds the visible area, in both the web app and the Telegram Mini App
### Definition of Done
- [x] All AC checked
...
### Changelog
- **Bug 1 (deletion):** root cause was `content_items.account_id` (and transitively `shortlist_items`/`deep_analysis_items` via `content_item_id`) having no `ON DELETE` cascade — deleting an account with any scrape history hit an unhandled `IntegrityError` → 500 → the frontend's generic error toast. First fix attempt cascade-deleted the child rows explicitly; superseded mid-session by direct user feedback ("archive instead") before it shipped to PROD.
- **Bug 2 (staleness):** `accounts` state in both `app/(app)/page.tsx` and the per-project `results/page.tsx` was fetched once on mount and never refreshed, so a competitor added elsewhere (or even via this same dialog) never appeared in the picker until a full page reload. Looked exactly like a scroll bug at first (short list, nothing to overflow) — confirmed via Chrome DevTools (`scrollHeight === clientHeight` even after adding more competitors) before finding the real cause.
- **Bug 3 (scroll) — the actual reported bug, took the longest to isolate:** in `RunDialog`'s competitor picker, the scroll container is itself `display:flex;flex-direction:column` (needed to lay out its own children as a column) *and* a flex item of its own parent (`flex-1 min-h-0 overflow-y-auto`). Its middle child — the rounded-border box wrapping competitor rows — had `overflow-hidden` with no explicit `min-height`. Per the flexbox spec, a flex item with non-`visible` overflow gets an automatic minimum size of 0, making it eligible for flex-shrink; since the outer container's height was fixed and the full row list didn't fit, the browser shrunk that inner box to fit — clipping rows via its own `overflow-hidden` — instead of letting the outer container's `overflow-y-auto` do the scrolling. This is why `scrollHeight`/`clientHeight` kept measuring equal even with a real overflowing list: the inner box had already silently absorbed the overflow by cropping itself. Two earlier fix attempts (CSS `min-h-0`/`flex-1` on the outer scroll container; `Telegram.WebApp.disableVerticalSwipes()` for the swipe-gesture theory) were real, defensible fixes for other failure modes but not the actual cause here — both are still in place as genuine hardening, just not what resolved the reported bug. `shrink-0` on the inner list box was the actual fix.
- **Deploy-pipeline gotcha (not a code bug):** after the shrink-0 fix, DEV kept serving stale bundles across several redeploys despite CI going green each time and the git commit being verifiably correct — traced to a stuck Railway/BuildKit layer cache reusing an identical container image digest across builds regardless of source changes (a known class of Railway platform issue). Worked around by setting a throwaway env var on the DEV `web` service to bust the cache (removed again once confirmed fixed) and redeploying manually with `railway up frontend --path-as-root --service web` — hit the same "plain `railway up` from a subdirectory uploads the whole monorepo root" gotcha `[E17-S10]` already documented for `backend`/`worker`, now confirmed to apply to `web` too.
### Handover
- `Account.archived_at` (nullable timestamptz, migration `a2b3c4d5e6f7`, now head) is the soft-delete flag, mirroring `Project.archived_at`'s existing pattern. `DELETE /projects/{id}/accounts/{account_id}` sets it instead of deleting the row; `GET /projects/{id}/accounts` filters `archived_at IS NULL`. `POST /projects/{id}/accounts` checks archived rows by `normalized_url` first and un-archives a match in place (same id, history intact) rather than erroring on the `(account_list_id, normalized_url)` unique constraint.
- The DB-level `enforce_account_list_cap` trigger (backstop for the 50-per-list cap, D13) is rescoped in the same migration to count only `archived_at IS NULL` rows — otherwise repeated archive/re-add cycles would eventually block adding genuinely new competitors even with a mostly-empty active list.
- Any future screen that opens `RunDialog`/`ScheduledRunDialog` with an `accounts` prop **must** refetch it immediately before opening, not rely on a value loaded earlier — both dialogs seed their local competitor-selection state from that prop exactly once per mount (deliberately, so a later prop update doesn't clobber the user's in-progress selection edits), so a stale prop at mount time never self-corrects. See `page.tsx:onPickRunType` and `results/page.tsx`'s create-run button for the pattern (`await api.listAccounts(...)` then `setAccounts(...)`, only then open the dialog).
- **New CSS gotcha for this codebase, worth remembering for any future bottom-sheet/scrollable-list layout:** a flex item with `overflow-hidden` (or any non-`visible` overflow) and no explicit `min-height`, nested directly inside another flex container that is *itself* height-constrained, is eligible to be crushed by flex-shrink instead of the intended scroll container ever overflowing. Give any such item `shrink-0` if it should never be compressed. `run-dialog.tsx` and `scheduled-run-dialog.tsx`'s competitor-list boxes both carry this now.
- `Telegram.WebApp.disableVerticalSwipes()` (Bot API 7.7+, `frontend/lib/telegram-webapp.ts:initTelegramWebApp`) and the `min-h-0 flex-1` fix on `BottomSheet`'s and `RunDialog`'s scroll containers remain in place — genuine hardening for other scroll-adjacent failure modes, not dead code, even though neither was the root cause of this story's reported bug.
- **Railway deploy note (extends `[E17-S10]`'s):** `railway up` from inside a service subdirectory can still upload the whole monorepo root if the local project link is rooted there — always use `railway up <dir> --path-as-root --service <name>` for any manual deploy of any of the three services (`api`, `worker`, `web`). Separately, if DEV keeps serving a stale bundle despite a green CI deploy and a verified-correct git commit, suspect a stuck Railway build-cache layer (identical `containerimage.digest` across builds) rather than the code — bust it by setting/changing any env var on the affected service, which Railway's build cache treats as a cache-busting event, then redeploy.
- ENV vars added: none permanent (a temporary `BUILD_CACHE_BUST` var was set and removed on the DEV `web` service during this story, purely as a cache-bust mechanism).
...
- [x] `get_engine()` sets explicit `pool_size`/`max_overflow` sized for expected concurrent API request volume, with headroom under the Postgres plan's `max_connections` — **confirmed 2026-08-03**: DEV's `max_connections=100` (Railway dashboard Postgres query tab), well above today's 40-connection worst case
...
3. `Settings.db_pool_size`/`db_max_overflow` (10/10 each) now flow into `get_engine()`'s `create_async_engine()` call. Querying `max_connections` needs `DATABASE_URL`, and fetching that was correctly blocked by the permission classifier as credential access this session — but the user confirmed it directly via the Railway dashboard's Postgres query tab shortly after this story closed: **DEV `max_connections=100`**. 10+10=20 per process × 2 processes (api, worker, each `numReplicas: 1`) = 40 total connections, comfortably under 100 — the chosen values are confirmed safe, no change needed.
...
### Acceptance Criteria
- [x] New user-level preference fields — `User.notify_review_enabled`/`notify_analysis_enabled` (migration `a3b4c5d6e7f8`), default `True` for both (preserves today's default-on behavior for existing accounts)
- [x] `GET`/`PATCH` on the user settings endpoint — reused the existing `GET /auth/me` (now returns both fields) plus a new `PATCH /auth/me/notifications` for writes, rather than overloading `PATCH /auth/me` (display-name-only contract, kept separate to avoid a breaking change to `DisplayNameIn` callers)
- [x] Settings page gets a new "Уведомления" section with the two toggles, following `handleSaveName`'s established saving/saved/error pattern (`saveNotifyPrefs`)
- [x] `run-dialog.tsx` and `scheduled-run-dialog.tsx`'s per-run notify toggle UI **removed** (not hidden — the user's own Handover note flagged this choice for a second look; going with removal since the field is now fully decorative), replaced with `notifySettingsNote`: "Уведомления настраиваются в разделе «Настройки»." Also removed the now-misleading per-schedule notify badge on the home feed and the project Scheduled Runs page (both previously rendered `sr.notify_enabled`, which no longer reflects the real firing decision)
- [x] Backend notify-decision path reads the new global per-type setting: new `notify_enabled_for_run_type(user, run_type)` helper in `telegram_notify.py` (`deep_analysis` → `notify_analysis_enabled`, else → `notify_review_enabled`), consumed by all 4 call sites in `worker.py` (the two `process_run` exception branches, `_finish_run`'s success path, `_notify_deep_analysis`). **Decision on the now-unused request fields: kept accepted-but-ignored** (`RunRequestIn.notify_on_complete`, `ScheduledRunIn.notify_enabled` both stay in the backend schema, just no longer read for the decision) — avoids a breaking API change for zero benefit, documented in both fields' own comments
- [x] Existing scheduled runs with a per-run `notify_enabled` already saved: no migration/backfill needed — the global setting wins outright as designed, the old per-schedule value is simply never consulted post-deploy (verified via a rewritten regression test, see Changelog)
- [x] `[E14-S6]`'s scheduled-run notify toggle and `[E22-S2]`'s run-now notify toggle are both fully superseded — their UI is gone from both dialogs
### Definition of Done
- [x] All AC checked
- [x] Tests written and passing — 393 backend tests (up from 385: 2 new `PATCH /auth/me/notifications` tests, 3 new `notify_enabled_for_run_type` unit tests, 1 rewritten scheduled-run notify-skip test); frontend `tsc --noEmit`/`next lint`/`next build` all clean
- [x] CI green (pending push) — ruff/ruff format/mypy clean locally; migration downgrade/upgrade round-trip verified against local Postgres (both the dedicated test DB and the local dev DB — see Changelog for why the dev DB needed a manual `alembic upgrade head` too)
- [ ] Smoke test passed — DEFERRED, see below
- [x] DONE.md updated
- [x] BACKLOG.md updated
### Smoke test
DEFERRED — per CLAUDE.md's no-agent-UI-testing constraint. Needs a real DEV pass: toggle each global Settings switch off, run one Review and one Analysis of each notify-state combination, confirm Telegram DMs arrive only when the corresponding global toggle is on, regardless of anything previously configured per-run/per-schedule (now impossible to configure anyway, since the UI is gone).
### Files to read
...
### Changelog
- Extracted the `ToggleSwitch` component (previously duplicated verbatim in both `run-dialog.tsx` and `scheduled-run-dialog.tsx`) into `frontend/components/ui/index.tsx` — both dialogs still use it for their unrelated repeat-mode toggle, and the Settings page needed one too; three call sites made this the right moment to de-duplicate rather than add a third copy.
- **Real gap found via a full-suite test run**: three `test_telegram_webapp.py` tests started failing after the migration with `column users.notify_review_enabled does not exist` — root cause is that those specific tests call the API through a raw `ASGITransport` client with no `get_session` dependency override, so they hit the app's *real* configured `DATABASE_URL` (the local dev Postgres `content_scout` DB) rather than the test suite's isolated `content_scout_test` DB the `migrated_db`/`session` fixtures manage. This is a pre-existing test-isolation quirk, not something this story introduced or fixed — noted here since it's the kind of thing `[E21-S6]` (this sprint, next-next) is meant to catch a *class* of, not just because it happened to surface now. Resolved practically by running `alembic upgrade head` against the local dev DB directly (verified downgrade/upgrade round-trip there too), which is what every migration story in this project's history has done when a local Postgres is available.
- Rewrote `test_scheduled_run_completion_skips_notify_when_disabled` (renamed `..._when_globally_disabled`) — its old premise (a schedule's own `notify_enabled=False` suppresses the DM) is exactly what this story replaces; now asserts the opposite mechanism (`owner.notify_review_enabled=False` suppresses it, even with the schedule's now-ignored field left `True`).
### Handover
Opened 2026-08-07 from direct first-user feedback (item 3 of a 4-item batch — see `[E8-S10]`, `[E18-S7]`, `[E8-S9]`'s Handover). Implemented as part of an autonomous back-to-back pass through Sprint 13 (direct user request: "start and complete sprint 13 without my involvement"). Went with full **removal** (not hiding) of the per-run toggle UI per the story's own AC wording ("removed (or hidden)") — the field is now fully inert server-side, so a hidden-but-still-submitting toggle would be actively misleading, not just redundant. This also removes `[E14-S6]`'s and `[E22-S2]`'s notify UI entirely; flagging that net effect again here in case a future review wants to revisit, exactly as the original Handover asked.
...
### Acceptance Criteria
- [x] Re-read `[E17-S12]`, `[E21-S4]`, `[E21-S5]` (and `[E17-S11]` for the prior cluster) together — common root shape across all four: each was a real gap in code that looked complete because it handled the one concrete failure shape a live incident happened to produce, without generalizing to the *class* of failure that shape belonged to. This audit found three more instances of exactly that pattern, one per AC item below
- [x] **Synthesis retry — real gap found and fixed.** `[E17-S12]`'s validation only checked that `stats`/`recommendations` were present dicts, not that either contained its OWN required nested fields (`stats.topics`, `recommendations.do_more`, etc. — `REPORT_TOOL`'s schema has two more levels of `required` lists it never checked). A response passing the loose check but missing a nested field would have shipped a report the frontend renders unconditionally as `stats.topics.length` — same crash class as the original bug, one level deeper, never actually closed. Fixed: new `_has_required_fields()` cross-references every candidate against `REPORT_TOOL["input_schema"]["properties"][...]["required"]` directly (so schema and validator can't drift apart again), reusing the exact same retry-then-fail flow. 3 new regression tests
- [x] **Hidden-account flow — real gap found and fixed.** `_resolve_or_create_account` had exactly one `Account(...)` construction site (confirmed — the only other one, `api/accounts.py`'s `add_accounts`, is the explicit-user-add path and already handles archived/hidden reactivation correctly). But its `existing`-reuse branch silently un-archived (`archived_at = None`) a match with no `hidden`-style safeguard — a deliberately-removed competitor whose handle happened to match a post-mode Analysis target would silently reappear in the visible competitor list, an implicit resurrection parallel to `[E21-S4]`'s original "new account" pollution bug, just for the "existing archived account" case instead. Verified nothing downstream in the post-mode pipeline (`GET /runs/{id}/accounts` returns `[]` for post mode; virality/export/notify queries don't filter on `archived_at` at all) actually needs the reactivation — removed it outright. 1 new regression test
- [x] **Pricing model — checklist run, one real gap found and fixed.** All 5 real `client.messages.create`/`.batches.create` call sites checked against their `usage_events` recording: `summarizer.py`'s per-item path (Haiku rate, correct), `run_summary.py` (Haiku rate + D52 base charge, correct), `deep_analysis_extraction.py` (Haiku rate, correct), `deep_analysis_synthesis.py` (Sonnet rate + D51 base charge, correct) — and `summarizer.py`'s Message Batches path (`_summarize_via_batches`, `summary_batch_threshold`+ items), which recorded `usage_events` at the **full non-batch rate** despite Anthropic's Batches API billing at half price, overstating real internal cost ~2x for every large Review run's admin/usage cost view. New `Settings.claude_batch_cost_multiplier` (0.5) applied at that one call site — internal cost-accounting fix only, does not touch `token_balance`/`tokens_charged` (what the user is billed). 1 extended regression test
- [x] Decision on regression-test category: **one-off tests, not a new category/harness.** All three findings were narrow enough to cover with the existing per-file fake-client/direct-DB-assertion pattern already used throughout `test_deep_analysis_synthesis.py`/`test_worker.py`/`test_summarizer.py` — no shared gap in test *infrastructure* was found, just three shared gaps in *validation completeness*, each closed at its own call site
### Definition of Done
- [x] All AC checked
- [x] Tests written and passing — 397 backend tests (up from 393: 3 nested-required-field tests, 1 archived-account test, 1 extended batch-pricing assertion). ruff/ruff format/mypy clean
- [x] CI green (pending push)
- [ ] Smoke test passed — DEFERRED, see below
- [x] DONE.md updated
- [x] BACKLOG.md updated
### Smoke test
DEFERRED — per CLAUDE.md's no-agent-UI-testing constraint. Needs a real DEV Analysis (account mode and post mode) confirming no regression in the three already-fixed behaviors, plus specifically: a run against an account whose handle was previously removed (archived) confirming it no longer resurfaces in the competitor list, and a large-enough Review run (≥`summary_batch_threshold` items) confirming the admin/usage cost view now shows the halved batch rate.
### Files to read
