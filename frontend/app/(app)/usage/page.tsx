...
export default function UsagePage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8">
      ...
      {!usage && !error && <p className="text-secondary">{t("loading")}</p>}

      {usage && (
        <section className="flex flex-col gap-4">
          <h2 className="text-base font-medium text-secondary">{t("currentMonth")}</h2>
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {usage.by_kind.map((row: KindTotalResponse) => (
                    <tr key={row.kind} className="border-t border-border">
                      <td className="whitespace-nowrap px-4 py-2 text-ink">{kindLabel(row.kind)}</td>
                ...
                <tfoot>
                  <tr className="border-t border-border">
                    <td colSpan={2} className="px-4 py-2 font-medium text-ink">
                      {t("totalCost")}
                    </td>
