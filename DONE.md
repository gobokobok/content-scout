...
**Smoke test:** DEFERRED — needs a real DEV deploy + live Apify console (pull post-fix run-detail pages to confirm `Memory: 256 MB` actually landed; enqueue >5 concurrent runs and confirm queueing behavior), same established pattern as every other Apify-account-dependent verification in this project. (Postgres `max_connections` confirmed separately, same day, at 100 — see Handover above; not part of this remaining deferral.)
