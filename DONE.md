...
- `Settings.db_pool_size`/`db_max_overflow` (10/10) now flow into `get_engine()`'s `create_async_engine()` call, replacing SQLAlchemy's unconfigured default (5+10=15). **Not derived from a real number**: the Railway Postgres plan's `max_connections` couldn't be checked this session — fetching `DATABASE_URL` to query it directly was correctly blocked as credential access by the permission classifier. 10+10=20/process × 2 processes (api, worker) = 40 total is a conservative, documented placeholder pending a real dashboard check.
...
**Smoke test:** DEFERRED — needs a real DEV deploy + live Apify console (pull post-fix run-detail pages to confirm `Memory: 256 MB` actually landed; enqueue >5 concurrent runs and confirm queueing behavior), same established pattern as every other Apify-account-dependent verification in this project. Postgres `max_connections` should be confirmed the same pass, before AC 3's 20-connection-per-process number is trusted beyond pilot scale.
...
**Smoke test:** DEFERRED — the 401 fix has partial real-world confirmation (user's retest showed clean logs, no more 401s), but the D38 auto-project change and its interaction with the now-removed prompt UI has not been retested live yet. Needs: a genuinely new Telegram account opening the Mini App on iOS, confirming the home feed is immediately usable (no prompt, "+" works right away) and Competitors resolves normally.
